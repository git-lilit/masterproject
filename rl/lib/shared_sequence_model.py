...
class SequenceModel(nn.Module):
    def __init__(
        self, vocab_size, embedding_dim, num_heads, output_dim, seq_len, num_final_heads, device="cpu"
    ):
        ...
        self.heads = nn.ModuleList([
            nn.Sequential(
                nn.Linear(self.hidden_dim, self.hidden_dim),  # First FC layer
                nn.ReLU(),
                nn.Linear(self.hidden_dim, output_dim)  # Second FC layer
            )
            for _ in range(num_final_heads)
        ])

        self.to(self.device)

    def forward(self, x, head_idx, seq_lens=None):
        ...
        output = self.heads[head_idx](start_token_output)
        return output
