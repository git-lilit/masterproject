...
class SequenceModel(nn.Module):
    def __init__(self, vocab_size, embedding_dim, num_heads, output_dim, seq_len, device="cpu"):
    ...
    def forward(self, x, seq_lens=None):
        ...
        seq_len = x.size(1)
        attn_mask = torch.triu(torch.ones(seq_len, seq_len), diagonal=1)  # Upper triangular mask
        ...
        attention_output, _ = self.attention(embeddings, embeddings, embeddings, key_padding_mask=key_padding_mask, attn_mask=attn_mask)
    ...
    def preprocess_input(self, x, seq_lens=None):
        ...
        if seq_lens is None:
            seq_lens = torch.sum(x != self.pad_token_id, dim=1) + 1  # Include start token
